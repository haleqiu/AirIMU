...
class SubT(Sequence):
    def __init__(
        ...
        **kwargs,
        ...
        mean_bias = kwargs.get("mean_bias", [0.0, 0.0, 0.0])
        ...
        self.data["g_b"] = self.interp_rot(
            self.data["time"],
            self.data["gt_time"],
            pp.so3(self.data["gyro_bias"]).Exp(),
        ).Log()
        self.data["a_b"] = self.interp_xyz(
            self.data["time"], self.data["gt_time"], self.data["acc_bias"]
        )
        ...
        self.data["mask"] = torch.ones(
            self.data["time"].shape[0], dtype=torch.bool
        ).double()
    ...
    def load_imu(self, folder):
        imu_data = np.loadtxt(
            os.path.join(folder, "imu_data/imu_data.csv"),
            dtype=float,
            delimiter=",",
            skiprows=1,
        )
        ...
        self.data["quat_imu"] = imu_data[:, 1:5]
        ...
        self.data["acc"] = torch.tensor(
            imu_data[:, 8:11]
        )  # acc a_RS_S_x [m s^-2],a_RS_S_y [m s^-2],a_RS_S_z [m s^-2]

    def load_gt(self, folder):
        gt_data = np.loadtxt(
            os.path.join(folder, "ground_truth/ground_truth_imu.csv"),
            dtype=float,
            delimiter=",",
            skiprows=1,
        )
